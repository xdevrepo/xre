...
.then(profiles => {
    if(!profiles) {
        errors.noprofile = ' There are no profiles at this times';
        return res.status(404).json(errors);
    }
...
.catch(err => res.status(404).json({ profile: 'There are no profiles at this times'})
);
...
router.delete(
    ...
    (req, res) => {
      ...
      Profile.findOne({ user: req.user.id }).then(profile => {
        const newExp = profile.education.filter(education => education.id !== req.params.edu_id);
   
        if (newExp.length === profile.education.length) {
